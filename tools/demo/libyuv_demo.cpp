...
const char* GetFormatName(PixelFormat format) {
    switch (format) {
        ...
        case FORMAT_YUV420_NV12: return "NV12";
        case FORMAT_YUV420_NV21: return "NV21";
...
struct ImageData {
    ...
    ImageData() : fmt(FORMAT_UNKNOWN), width(0), height(0) {
        stride[0] = stride[1] = 0;
    }

    void PrintInfo(const char* prefix) const {
        std::cout << "[" << prefix << "] Format: " << GetFormatName(fmt)
...
ImageData BGR24ToNV12(const ImageData& bgr24) {
    ...
    libyuv::RGB24ToARGB(
        bgr24.plane[0].data(), bgr24.stride[0],
        argb_buffer.data(), argb_stride,
        bgr24.width, height);

    // ARGB -> NV12
    libyuv::ARGBToNV12(
        argb_buffer.data(), argb_stride,
        img.plane[0].data(), img.stride[0],
        img.plane[1].data(), img.stride[1],
        bgr24.width, height);
...
ImageData RGB24ToNV12(const ImageData& rgb24) {
    ...
    libyuv::RAWToARGB(
        rgb24.plane[0].data(), rgb24.stride[0],
        argb_buffer.data(), argb_stride,
        rgb24.width, height);
